...
describe('YAML Generator', () => {
  ...
  it('should add appropriate comments based on mode', () => {
    const config: KometaConfig = { settings: { cache: true } };

    const templateYaml = generateYaml({ config, mode: 'template' });
    expect(templateYaml).toContain('Template (no secrets)');

    const maskedYaml = generateYaml({ config, mode: 'masked' });
    expect(maskedYaml).toContain('secrets masked');

    const fullYaml = generateYaml({ config, mode: 'full' });
    expect(fullYaml).toContain('WARNING');
  });

  it('should omit comments when includeComment is false', () => {
    const config: KometaConfig = { settings: { cache: true } };

    const yaml = generateYaml({ config, mode: 'template', includeComment: false });
