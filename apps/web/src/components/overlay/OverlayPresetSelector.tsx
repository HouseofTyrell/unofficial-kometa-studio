...
// Group presets by category for organized dropdown
...
export function OverlayPresetSelector({
  ...
  const presetsByCategory = PRESET_CATEGORIES.reduce(
    (acc, category) => {
      acc[category] = OVERLAY_PRESETS.filter((p) => p.category === category);
      return acc;
    },
    {} as Record<string, OverlayPreset[]>
  );
